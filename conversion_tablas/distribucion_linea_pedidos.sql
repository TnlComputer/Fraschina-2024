...
--     id,
--     pedido_id,  -- Renombrado de 'pedido_id' a 'pedido_nro'
--     fecha,
--     producto_id,
--     cantidad,
--     precio_unitario,
...
SELECT
    id_linea AS id,
    CAST(nroPed AS UNSIGNED) AS pedido_nro,
    STR_TO_DATE(fechaPed, '%Y-%m-%d') AS fecha,
    CAST(producto_id AS UNSIGNED) AS producto_id,
    CAST(
        COALESCE(NULLIF(cantidad, ''), '0') AS UNSIGNED
    ) AS cantidad,
    CAST(
        COALESCE(
            NULLIF(
    ...
    linea,
    bandera,
    CAST(idCliente AS UNSIGNED) AS distribucion_id,
    ...
    CAST(orden_Ped AS UNSIGNED) AS orden,
...
WHERE
    ...
    AND fecEntrega IS NOT NULL
    AND fecFac_Ped IS NOT NULL
    AND CAST(nroPed AS UNSIGNED) IN (
        SELECT id
