  ...
  <div class="row">
    <div class="col-12">
      <div class="card-header">
        <h3 class="card-title">{{ __('Modos') }}</h3>
      </div>
      <div class="col-12">
        <div class="py-2">
              ...
              <div class="p-2 text-gray-900 text-left text-xs">
                <div class="barra__index d-flex justify-content-between align-items-center mb-3">
                  <div class="div__nuevo">
                    <form action="{{ route('modos.create') }}">
                      <input class="btn btn-primary" type="submit" value="Nuevo">
                    </form>
                  </div>
                  <div class="div__buscar d-flex">
                    <form method="get" action="{{ route('modos.index') }}" class="form__buscar d-flex">
                      @csrf
                ...
                <table class="table table-sm table-striped table-bordered text-md">
                  ...
                  <tr>
                    <td>{{ $modo->nombre }}</td>
                    {{-- <td class="text-center">{{ $modo->colorAcc }}</td>
                    <td class="text-center">{{ $modo->colorCod }}</td>
                    <td class="text-center">{{ $modo->status }}</td> --}}
                    <td class="d-flex gap-1 justify-content-center">
                      <a href="{{ route('modos.edit', $modo->id) }}" class="btn btn-warning btn-xs" title="Editar">
                        <i class="fa-solid fa-pen-to-square fa-xs"></i>
                      </a>
                      <form method="POST" action="{{ route('modos.destroy', $modo->id) }}"
