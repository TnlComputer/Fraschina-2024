...
<div class="row">
  <div class="col-md-12 mb-3">
    <label for="molino_id" class="form-label">Molino</label>
    <select name="molino_id" id="molino_id" class="form-control @error('molino_id') is-invalid @enderror" required>
      <option value="">Seleccione un Molino</option>
      @foreach ($molinos as $molino)
      <option value="{{ $molino->id }}" {{ old('molino_id', $personal->molino_id ?? '') ==
        $molino->id ? 'selected' : '' }}>
        {{ $molino->razonsocial }}
      </option>
      @endforeach
