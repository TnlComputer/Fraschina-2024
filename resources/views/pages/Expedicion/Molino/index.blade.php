              ...
              <div class="p-2 text-gray-900 text-left text-xs">
                <div class="barra__index d-flex justify-content-between align-items-center mb-3">
                  <div class="div__nuevo">
                    <form action="{{ route('expedicion_molinos.create') }}">
                      <input class="btn btn-primary" type="submit" value="Nuevo">
                    </form>
                  </div>
                  <div class="div__buscar d-flex">
                    <form method="get" action="{{ route('expedicion_molinos.index') }}" class="form__buscar d-flex">
                      @csrf
                ...
                <table class="table table-sm table-striped table-bordered w-100">
                  ...
                  @forelse($expedicion_molinos as $expedicion_molino)
                  <tr>
                    <td>{{ $expedicion_molino->nroMolino_id }}</td>
                    <td>{{ $expedicion_molino->calificacion }}</td>
                    <td>{{ $expedicion_molino->sigla }}</td>
                    <td>{{ $expedicion_molino->nroUPed }}</td>
                    <td>{{ $expedicion_molino->contMolino }}</td>
